...
print('sys.path')
print(sys.path)
from elasticsearch_agent.agent.agent_factory import agent_factory, call_with_messages
...
def test_max_age():
    execution_template("Which document has the biggest age?")
...
# """)
    res = call_with_messages(
        react_system_prompt="""确保首先查询 ElasticSearch 数据库中的索引。
确保在查询索引后查询字段名称。
必须调用工具才能回答这个问题。""",
        user_input=question,
    )
#     res = agent_executor.run(f"""
# 确保首先查询 ElasticSearch 数据库中的索引。
# 确保在查询索引后查询字段名称。
# 必须调用工具才能回答这个问题。           
# 然后回答这个问题：:
# {question}
